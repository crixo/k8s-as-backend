/*
Copyright The Kubernetes Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package v1

import (
	"time"

	v1 "github.com/crixo/k8s-as-backend/library/pkg/apis/k8sasbackend/v1"
	scheme "github.com/crixo/k8s-as-backend/library/pkg/client/clientset/versioned/scheme"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	rest "k8s.io/client-go/rest"
)

// TodosGetter has a method to return a TodoInterface.
// A group's client should implement this interface.
type TodosGetter interface {
	Todos(namespace string) TodoInterface
}

// TodoInterface has methods to work with Todo resources.
type TodoInterface interface {
	Create(*v1.Todo) (*v1.Todo, error)
	Update(*v1.Todo) (*v1.Todo, error)
	Delete(name string, options *metav1.DeleteOptions) error
	DeleteCollection(options *metav1.DeleteOptions, listOptions metav1.ListOptions) error
	Get(name string, options metav1.GetOptions) (*v1.Todo, error)
	List(opts metav1.ListOptions) (*v1.TodoList, error)
	Watch(opts metav1.ListOptions) (watch.Interface, error)
	Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v1.Todo, err error)
	TodoExpansion
}

// todos implements TodoInterface
type todos struct {
	client rest.Interface
	ns     string
}

// newTodos returns a Todos
func newTodos(c *K8sasbackendV1Client, namespace string) *todos {
	return &todos{
		client: c.RESTClient(),
		ns:     namespace,
	}
}

// Get takes name of the todo, and returns the corresponding todo object, and an error if there is any.
func (c *todos) Get(name string, options metav1.GetOptions) (result *v1.Todo, err error) {
	result = &v1.Todo{}
	err = c.client.Get().
		Namespace(c.ns).
		Resource("todos").
		Name(name).
		VersionedParams(&options, scheme.ParameterCodec).
		Do().
		Into(result)
	return
}

// List takes label and field selectors, and returns the list of Todos that match those selectors.
func (c *todos) List(opts metav1.ListOptions) (result *v1.TodoList, err error) {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
	result = &v1.TodoList{}
	err = c.client.Get().
		Namespace(c.ns).
		Resource("todos").
		VersionedParams(&opts, scheme.ParameterCodec).
		Timeout(timeout).
		Do().
		Into(result)
	return
}

// Watch returns a watch.Interface that watches the requested todos.
func (c *todos) Watch(opts metav1.ListOptions) (watch.Interface, error) {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
	opts.Watch = true
	return c.client.Get().
		Namespace(c.ns).
		Resource("todos").
		VersionedParams(&opts, scheme.ParameterCodec).
		Timeout(timeout).
		Watch()
}

// Create takes the representation of a todo and creates it.  Returns the server's representation of the todo, and an error, if there is any.
func (c *todos) Create(todo *v1.Todo) (result *v1.Todo, err error) {
	result = &v1.Todo{}
	err = c.client.Post().
		Namespace(c.ns).
		Resource("todos").
		Body(todo).
		Do().
		Into(result)
	return
}

// Update takes the representation of a todo and updates it. Returns the server's representation of the todo, and an error, if there is any.
func (c *todos) Update(todo *v1.Todo) (result *v1.Todo, err error) {
	result = &v1.Todo{}
	err = c.client.Put().
		Namespace(c.ns).
		Resource("todos").
		Name(todo.Name).
		Body(todo).
		Do().
		Into(result)
	return
}

// Delete takes name of the todo and deletes it. Returns an error if one occurs.
func (c *todos) Delete(name string, options *metav1.DeleteOptions) error {
	return c.client.Delete().
		Namespace(c.ns).
		Resource("todos").
		Name(name).
		Body(options).
		Do().
		Error()
}

// DeleteCollection deletes a collection of objects.
func (c *todos) DeleteCollection(options *metav1.DeleteOptions, listOptions metav1.ListOptions) error {
	var timeout time.Duration
	if listOptions.TimeoutSeconds != nil {
		timeout = time.Duration(*listOptions.TimeoutSeconds) * time.Second
	}
	return c.client.Delete().
		Namespace(c.ns).
		Resource("todos").
		VersionedParams(&listOptions, scheme.ParameterCodec).
		Timeout(timeout).
		Body(options).
		Do().
		Error()
}

// Patch applies the patch and returns the patched todo.
func (c *todos) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v1.Todo, err error) {
	result = &v1.Todo{}
	err = c.client.Patch(pt).
		Namespace(c.ns).
		Resource("todos").
		SubResource(subresources...).
		Name(name).
		Body(data).
		Do().
		Into(result)
	return
}
